...
Route this request to the `codex-jr:codex-rescue` subagent.
...
- If the request includes `--background`, run the `codex-jr:codex-rescue` subagent in the background.
- If the request includes `--wait`, run the `codex-jr:codex-rescue` subagent in the foreground.
...
- Do not ask the subagent to inspect files, monitor progress, poll `/codex-jr:status`, fetch `/codex-jr:result`, call `/codex-jr:cancel`, summarize output, or do follow-up work of its own.
...
- If the helper reports that Codex is missing or unauthenticated, stop and tell the user to run `/codex-jr:setup`.
